...
class ItemLogController {
    ...
    async addItemLog(req, res, next){
        ...
        this.connection.query('INSERT INTO log(_fk_item_editorial, lan_id, time_stamp, event, details, user_name) VALUES(\' ' + [_pk_item_editorial] + ' \',\' ' + Math.random() + '\',DATE_FORMAT(NOW(), \'%Y-%m-%d %T\'),\' ' + [reason] + '\',\'null\',\'generic_user\')' ,(err, result) => {
                if(err){
