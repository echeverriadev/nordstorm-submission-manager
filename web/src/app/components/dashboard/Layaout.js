...
const Layaout = (props) => {
  const { isChangingFilter, classes, onEditKeyPress, onChange, items, addItem, onAddChange, onSubmit, onChangeFilter,
        ...
        <Grid>
              {
                items.map((item, index) => (
                  <CardCell key={index} index={index} item={item} isChangingFilter={isChangingFilter} onEditKeyPress={onEditKeyPress} onChange={onChange} cycles={cycles} onDeleteItem={onDeleteItem} onDuplicateItem={onDuplicateItem} />
                ))
              }
              {
